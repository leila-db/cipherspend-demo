...
- **[NiobiumInc/niobium-skills](https://github.com/NiobiumInc/niobium-skills)** — the
  `fhe-application-design` skill, **FHEanna**. CipherSpend was designed using this
  methodology: every source file in this repository is headed *"Generated by the Niobium
  FHE Application Design AI Assistant (FHEanna)"*. It set the hard crypto constraints
  CipherSpend follows: CKKS, ring dimension `2^16`, ≥128-bit security, **no bootstrapping
  (multiplicative depth 0)**, a fixed rotation-key set, no relinearization,
  secret-key-client-only, and a semi-honest threat model. This shaped the design only — it
  is **not** a runtime dependency (see "Used today"). See
...
- **[SECURITY_REVIEW.md](SECURITY_REVIEW.md)** — the pre-publication security & data-leak
...
keys, ciphertexts, decrypted results, or caches are included or tracked. See
